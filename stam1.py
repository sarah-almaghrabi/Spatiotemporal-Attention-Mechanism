...
def model(Tx, Ty, inp_var, h_s, dropout):
    ...
    encoder_input = Input(shape = (Tx, inp_var),name='encoder_input')   # (None, 10, 8)
    spatial_input = Input(shape = (inp_var, Tx), name='spatial_input' )    # (None, 8, 10)
    
    # Initialize
    s0 = Input(shape=(h_s,), name= 's0')  # Initialize hidden state for decoder   (None, 16)
    c0 = Input(shape=(h_s,), name= 'c0')  # Initialize cell state for decoder     (None, 16)
    yhat0 = Input(shape=(1, ), name='yhat0')  # Initialize prev pred y   (None, 1)
...
plot_model(pred_model, to_file='model_plot.png', show_shapes=True, show_layer_names=True  )
...
print('y_train_hat_betas_avg:',y_train_hat_betas_avg)
print('y_val_hat_betas_avg:',y_val_hat_betas_avg)
print('y_test_hat_betas_avg:',y_test_hat_betas_avg)


print('y_train_hat_alphas:',y_train_hat_alphas)
